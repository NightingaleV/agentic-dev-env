## GUIDING PRINCIPLES for Formatting Databricks Notebooks (`py` files)
...
### Script Start
It must start with `# Databricks notebook source` on the first line, no empty spaces before. So begin the notebook with:
...
Use DBTITLE (if we work with databricks format) for Subsections. Add titles to the cell at the top of each code cell to indicate its purpose. These are essentially subsections.
