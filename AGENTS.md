...
1. **Base Content Processing**: Files from `src/base/` are copied to all targets with:
   - Folder name mapping (e.g., `prompts` → `command` for OpenCode)
   - File suffix transformation (e.g., `.md` → `.agent.md` for GitHub agents)
...
.github:
  folder_mappings:
    prompts: prompts
  file_suffix_rules:
    prompts: ".prompt.md"    # test.md → test.prompt.md
    agents: ".agent.md"      # debug.md → debug.agent.md
```
...
📖 **For detailed template usage, see [TEMPLATE_GUIDE.md](./TEMPLATE_GUIDE.md)**